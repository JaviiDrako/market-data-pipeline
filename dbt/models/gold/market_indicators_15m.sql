...
WITH source AS (
    SELECT * FROM {{ ref('market_candles_15m') }}
    {% if is_incremental() %}
        WHERE open_time >= (SELECT COALESCE(MAX(open_time) - INTERVAL '45 days', TIMESTAMP '1970-01-01') FROM {{ this }})
    {% endif %}
)
