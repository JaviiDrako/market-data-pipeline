...
-- Consumes ONLY market_indicators_1d + market_features_1d + market_signals_1d.
-- Contains keys + all Features + all Signals. No indicators, no OHLCV.
-- Extremely simple orchestration: no calculations.

WITH features AS (
    SELECT *
    FROM {{ ref('market_features_1d') }}
    {% if is_incremental() %}
        WHERE open_time >= (
            SELECT COALESCE(MAX(open_time) - INTERVAL '400 days', TIMESTAMP '1970-01-01')
            FROM {{ this }}
        )
    {% endif %}
),
