...
-- Consumes ONLY market_indicators_15m + market_features_15m + market_signals_15m.
-- Contains keys + all Features + all Signals. No indicators, no OHLCV.
-- Extremely simple orchestration: no calculations.

WITH features AS (
    SELECT *
    FROM {{ ref('market_features_15m') }}
    {% if is_incremental() %}
        WHERE open_time >= (
            SELECT COALESCE(MAX(open_time) - INTERVAL '45 days', TIMESTAMP '1970-01-01')
            FROM {{ this }}
        )
    {% endif %}
),
